...
export function FactionUpload({ onClose, onSuccess }: FactionUploadProps) {
  ...
  const [pendingFactionId, setPendingFactionId] = useState<string | null>(null)
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (result.wasOverwrite) {
        // Show overwrite confirmation (but we already uploaded, so just notify)
        // For better UX, we could pre-check before uploading
      }
  ...
  const handleConfirmOverwrite = async () => {
    ...
    try {
      const result = await uploadFaction(pendingFile)
      onSuccess?.(result.factionId)
      onClose()
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to upload faction')
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4">
        <h2 className="text-xl font-semibold text-white mb-4">Upload Faction</h2>

        {showOverwriteConfirm ? (
          <div>
            <p className="text-gray-300 mb-4">
              A local faction with ID "{pendingFactionId}" already exists. Do you want to overwrite it?
            </p>
