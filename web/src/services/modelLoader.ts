...
/**
 * The faction version a unit's models were actually built from, when that is NOT
 * the version being viewed.
 *
 * Model bundles are regenerated by a heavy manual workflow while faction data
 * refreshes daily, so the manifest falls back to a faction's newest bundle for
 * versions that have none of their own. Returns the bundle's version in that
 * case so the viewer can say where the model came from, and `null` when the
 * model belongs to the viewed version (or in dev, where models are unversioned).
 *
 * Reads the already-loaded manifest — no extra network request.
 */
export async function getModelBundleSourceVersion(
  factionId: string,
  version?: string | null
): Promise<string | null> {
  if (isDevLocalModels()) return null

  const manifestEntry = version
    ? await getManifestVersion(factionId, version)
    : await getManifestEntry(factionId)

  return manifestEntry?.models?.builtFromVersion ?? null
}
